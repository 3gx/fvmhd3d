...
#define BND_RADIUS  0.05
#define BND_RADIUS0 0.01
#define BND_RADIUS1 0.5
...
#define B0     (1.0*std::sqrt(2.0*D0*sqr(iso_cs)/beta))
// #define B0 0.0
#endif
...
//      p.vel = 0.0;
  ...
  void system::predict(const int i)
  {
...
#endif
#if 0
    ptcl_local[i].vel = ptcl_local[i].orig_vel = 0.0;
    const real dt = t_global - ptcl_local[i].tlast;
    ptcl_local[i].vel = ptcl_local[i].orig_vel + 0.0*ptcl_local[i].acc*dt;
    ptcl_local[i].pos = 
      ptcl_local[i].orig_pos  +
      (ptcl_local[i].orig_vel +
       ptcl_local[i].acc*(dt*0.0))*dt;
    ptcl_local[i].pos = periodic(ptcl_local[i].pos);
#endif
#if 1
    const real dt = t_global - ptcl_local[i].tlast;
    const vec3 rv = ptcl_local[i].orig_pos.cross(ptcl_local[i].vel);
    if (rv.abs() == 0.0) 
    {
      ptcl_local[i].pos = ptcl_local[i].orig_pos;
    ...
    const vec3 r(ptcl_local[i].orig_pos);
    const vec3 v(ptcl_local[i].orig_vel);
    const real th = v.abs()*dt/r.abs();
    ...
    if (!(std::abs(r.abs() - r1.abs()) <= 1.0e-10*r.abs()))
    {
      fprintf(stderr, " myproc= %d : i= %d  r1= %g  r0= %g  :: diff= %g  %g \n",
          myproc, i, r1.abs(), r.abs(), 
          r1.abs() - r.abs(), (r1 - r).abs());
    }
    assert(std::abs(r.abs() - r1.abs()) <=  1.0e-10*r.abs());
